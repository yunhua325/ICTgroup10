...
def read_probability_distribution(input_file):
            ...
            for row_num, row in enumerate(reader, 1):
                if len(row) >= 2:
                    ...
                    if 0 <= symbol <= 255:
                        ...
                        print(f"警告：第{row_num}行符号值{symbol}超出范围(0-255)，已忽略")
                else:
                    print(f"警告：第{row_num}行格式不正确，已跳过")
    ...
    # 检查是否读取到有效数据
    if symbol_count == 0:
        print("错误：CSV文件中没有有效的概率数据")
        sys.exit(1)
    
    # 验证概率分布
    total_prob = np.sum(probabilities)
    print(f"概率分布总和：{total_prob:.6f}")
    
    if abs(total_prob - 1.0) > 1e-6:
        print(f"警告：概率分布总和为 {total_prob:.6f}，不等于1.0")
        # 归一化
...
def write_message(message, output_file):
    ...
    if os.path.exists(output_file):
        print(f"警告：输出文件 {output_file} 已存在，将被覆盖")
    ...
    try:
        ...
        print(f"文件大小：{len(message_bytes)} 字节")
