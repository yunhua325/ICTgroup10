...
def encode(pmf_file_name, in_file_name, out_file_name):
    ...
    codebook = codec.get_code_table()  # 获取编码表（码本）
    ...
    header[0:2] = len(header).to_bytes(2, byteorder)  # 将头部大小写入头部的前2个字节
    # 将头部和编码数据写入输出文件
    with open(out_file_name, 'wb') as out_file:
        out_file.write(header)
        out_file.write(encoded)
    return (len(source), len(encoded))  # 返回源数据长度和编码后数据长度
...
def decode(in_file_name, out_file_name):
    ...
    decoded = np.asarray(codec.decode(encoded))[:source_len]   # 解码数据并截取到源数据长度
    decoded.tofile(out_file_name)  # 将解码数据写入输出文件
    # 返回编码数据长度和解码数据长度
